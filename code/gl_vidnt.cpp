...
cvar_t		vid_mode = {"vid_mode","4", false};
// Note that 0 is MODE_WINDOWED
cvar_t		_vid_default_mode = {"_vid_default_mode","0", true};
// Note that 3 is MODE_FULLSCREEN_DEFAULT
cvar_t		_vid_default_mode_win = {"_vid_default_mode_win","3", true};
cvar_t		vid_wait = {"vid_wait","0"};
cvar_t		vid_window = { "vid_window","1", true };
...
qboolean VID_SetWindowedMode (int modenum)
{
	...
	if (DIBWidth < 1024 || DIBHeight < 768)
	{
		DIBWidth = 1024;
		DIBHeight = 768;
		WindowRect.right = DIBWidth;
		WindowRect.bottom = DIBHeight;
	}
...
void	VID_Init (unsigned char *palette)
{
	...
	Cvar_RegisterVariable(&vid_window);
	...
	if (COM_CheckParm("-window") || vid_window.value > 0.0f)
	{
		hdc = GetDC (NULL);
