...
Goes to a _new map, taking all clients along
==================
*/
void Host_Changelevel_f (void)
{
	...
	if (Cmd_Argc() < 2)
	{
		Con_Printf ("changelevel <levelname> : continue game on a _new level\n");
		return;
	}
...
{
	SCR_BeginLoadingPlaque ();
	cls.signon = 0;		// need _new connection messages
}
...
void Host_Changelevel2_f (void)
{
	...
	if (Cmd_Argc() < 2)
	{
		Con_Printf ("changelevel2 <levelname> : continue game on a _new level in the unit\n");
		return;
	}
	if (!sv.active || cls.demoplayback)
	{
