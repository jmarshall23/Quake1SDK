...
static int net_acceptsocket = -1;		// socket for fielding _new connections
...
// so we can run on Win 3.1, where there isn't necessarily Winsock)
	...
	pWSAStartup = (int (WSAAPI*)(WORD, LPWSADATA)) GetProcAddress(hInst, "WSAStartup");
	pWSACleanup = (int (WSAAPI*)(void)) GetProcAddress(hInst, "WSACleanup");
	pWSAGetLastError = (int (WSAAPI*)(void)) GetProcAddress(hInst, "WSAGetLastError");
	psocket = (SOCKET(WSAAPI*)(int, int, int)) GetProcAddress(hInst, "socket");
	pioctlsocket = (int (WSAAPI*)(SOCKET, long, u_long*)) GetProcAddress(hInst, "ioctlsocket");
	psetsockopt = (int (WSAAPI*)(SOCKET, int, int, const char*, int)) GetProcAddress(hInst, "setsockopt");
	precvfrom = (int (WSAAPI*)(SOCKET, char*, int, int, struct sockaddr*, int*)) GetProcAddress(hInst, "recvfrom");
	psendto = (int (WSAAPI*)(SOCKET, const char*, int, int, const struct sockaddr*, int)) GetProcAddress(hInst, "sendto");
	pclosesocket = (int (WSAAPI*)(SOCKET)) GetProcAddress(hInst, "closesocket");
	pgethostname = (int (WSAAPI*)(char*, int)) GetProcAddress(hInst, "gethostname");
	pgethostbyname = (struct hostent* (WSAAPI*)(const char*)) GetProcAddress(hInst, "gethostbyname");
	pgethostbyaddr = (struct hostent* (WSAAPI*)(const char*, int, int)) GetProcAddress(hInst, "gethostbyaddr");
	pgetsockname = (int (WSAAPI*)(SOCKET, struct sockaddr*, int*)) GetProcAddress(hInst, "getsockname");
